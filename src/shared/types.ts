// PoC 共用領域型別（移植自 uber_payment_poc，commit 739d9af；Item 2 人類移植）。
// 詞彙定義見 uber_payment_poc 的 CONTEXT.md。
...
export interface Batch {
  batchId: string;
  accountId: string;
  windowStart: number; // 由權威時鐘裁定的窗口起點（ms）
...
// 推入全域佇列的任務（一個 batch = 同帳戶同窗口的多筆交易）
...
// 交易/批次的狀態機（見 uber_payment_poc 的 CONTEXT.md 與 spec）
